...
- **`kewlfft.aur` is installed unconditionally on both OSes** (bootstrap.sh step 4), even though the playbook only *uses* it for Arch's AUR tasks. Ansible resolves every task's module up front regardless of `when` conditions, so the playbook fails to parse on macOS without it. Don't gate this install to Arch only.
